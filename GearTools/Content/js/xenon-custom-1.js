...
{var $this=$(el),opts={allowClear:attrDefault($this,'allowClear',false),dropdownAutoWidth:true};$this.select2(opts);$this.addClass('visible');});if($.isFunction($.fn.niceScroll))
...
{ev.preventDefault();if(public_vars.$sidebarMenu.hasClass('collapsed')&&$li.parent().is('.main-menu'))
{var is_touch=(('ontouchstart'in window)||(navigator.MaxTouchPoints>0)||(navigator.msMaxTouchPoints>0));if(is_touch){$li.siblings().not($li).find('> a').data('urlJump',0);if($a.data('urlJump')>1){window.location.href=$a.attr('href');}
if(typeof $a.data('urlJump')=='undefined'){$a.data('urlJump',1);}else{$a.data('urlJump',$a.data('urlJump')+1);}}else{window.location.href=$a.attr('href');}
return;}
if(toggle_others)
...
{$li.data('is-busy',false).removeClass('opened');$sub.removeAttr('style');$sub_items.removeClass('hidden-item');$li.find('li.expanded ul').attr('style','').hide().parent().removeClass('expanded');ps_update(true);}});}
